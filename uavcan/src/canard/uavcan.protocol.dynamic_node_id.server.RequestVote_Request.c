#include <canard/uavcan.protocol.dynamic_node_id.server.RequestVote_Request.h>
#include <string.h>

static uint32_t encode_func(void* buffer, void* msg) {
    return encode_uavcan_protocol_dynamic_node_id_server_RequestVote_req(buffer, msg);
}

static uint32_t decode_func(CanardRxTransfer* transfer, void* msg) {
    return decode_uavcan_protocol_dynamic_node_id_server_RequestVote_req(transfer, msg);
}

const struct uavcan_message_descriptor_s uavcan_protocol_dynamic_node_id_server_RequestVote_req_descriptor = {
    ...
    sizeof(struct uavcan_protocol_dynamic_node_id_server_RequestVote_req_s),
...
uint32_t encode_uavcan_protocol_dynamic_node_id_server_RequestVote_req(struct uavcan_protocol_dynamic_node_id_server_RequestVote_req_s* msg, uint8_t* buffer) {
    uint32_t bit_ofs = 0;
    memset(buffer, 0, UAVCAN_PROTOCOL_DYNAMIC_NODE_ID_SERVER_REQUESTVOTE_REQ_MAX_PACK_SIZE);
    _encode_uavcan_protocol_dynamic_node_id_server_RequestVote_req(buffer, &bit_ofs, msg, true);
    return (bit_ofs+7)/8;
}
...
uint32_t decode_uavcan_protocol_dynamic_node_id_server_RequestVote_req(const CanardRxTransfer* transfer, struct uavcan_protocol_dynamic_node_id_server_RequestVote_req_s* msg) {
    uint32_t bit_ofs = 0;
    _decode_uavcan_protocol_dynamic_node_id_server_RequestVote_req(transfer, &bit_ofs, msg, true);
    return (bit_ofs+7)/8;
}
