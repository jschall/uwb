#include <canard/uavcan.protocol.RestartNode_Response.h>
#include <string.h>

static uint32_t encode_func(void* buffer, void* msg) {
    return encode_uavcan_protocol_RestartNode_res(buffer, msg);
}
...
const struct uavcan_message_descriptor_s uavcan_protocol_RestartNode_res_descriptor = {
    UAVCAN_PROTOCOL_RESTARTNODE_RES_DT_SIG,
    UAVCAN_PROTOCOL_RESTARTNODE_RES_DT_ID,
    CanardTransferTypeResponse,
    sizeof(struct uavcan_protocol_RestartNode_res_s),
...
uint32_t encode_uavcan_protocol_RestartNode_res(struct uavcan_protocol_RestartNode_res_s* msg, uint8_t* buffer) {
    uint32_t bit_ofs = 0;
    memset(buffer, 0, UAVCAN_PROTOCOL_RESTARTNODE_RES_MAX_PACK_SIZE);
    _encode_uavcan_protocol_RestartNode_res(buffer, &bit_ofs, msg, true);
    return (bit_ofs+7)/8;
}
