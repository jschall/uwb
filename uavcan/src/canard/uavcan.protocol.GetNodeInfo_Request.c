#include <canard/uavcan.protocol.GetNodeInfo_Request.h>
#include <string.h>

static uint32_t encode_func(void* buffer, void* msg) {
    return encode_uavcan_protocol_GetNodeInfo_req(buffer, msg);
}
...
const struct uavcan_message_descriptor_s uavcan_protocol_GetNodeInfo_req_descriptor = {
    UAVCAN_PROTOCOL_GETNODEINFO_REQ_DT_SIG,
    UAVCAN_PROTOCOL_GETNODEINFO_REQ_DT_ID,
    CanardTransferTypeRequest,
    sizeof(struct uavcan_protocol_GetNodeInfo_req_s),
...
uint32_t encode_uavcan_protocol_GetNodeInfo_req(struct uavcan_protocol_GetNodeInfo_req_s* msg, uint8_t* buffer) {
    uint32_t bit_ofs = 0;
    memset(buffer, 0, UAVCAN_PROTOCOL_GETNODEINFO_REQ_MAX_PACK_SIZE);
    _encode_uavcan_protocol_GetNodeInfo_req(buffer, &bit_ofs, msg, true);
    return (bit_ofs+7)/8;
}
