#include <canard/uavcan.protocol.AccessCommandShell_Request.h>
#include <string.h>

static uint32_t encode_func(void* buffer, void* msg) {
    return encode_uavcan_protocol_AccessCommandShell_req(buffer, msg);
}

static uint32_t decode_func(CanardRxTransfer* transfer, void* msg) {
    return decode_uavcan_protocol_AccessCommandShell_req(transfer, msg);
}

const struct uavcan_message_descriptor_s uavcan_protocol_AccessCommandShell_req_descriptor = {
    UAVCAN_PROTOCOL_ACCESSCOMMANDSHELL_REQ_DT_SIG,
    UAVCAN_PROTOCOL_ACCESSCOMMANDSHELL_REQ_DT_ID,
    CanardTransferTypeRequest,
    sizeof(struct uavcan_protocol_AccessCommandShell_req_s),
...
uint32_t encode_uavcan_protocol_AccessCommandShell_req(struct uavcan_protocol_AccessCommandShell_req_s* msg, uint8_t* buffer) {
    uint32_t bit_ofs = 0;
    memset(buffer, 0, UAVCAN_PROTOCOL_ACCESSCOMMANDSHELL_REQ_MAX_PACK_SIZE);
    _encode_uavcan_protocol_AccessCommandShell_req(buffer, &bit_ofs, msg, true);
    return (bit_ofs+7)/8;
}
...
uint32_t decode_uavcan_protocol_AccessCommandShell_req(const CanardRxTransfer* transfer, struct uavcan_protocol_AccessCommandShell_req_s* msg) {
    uint32_t bit_ofs = 0;
    _decode_uavcan_protocol_AccessCommandShell_req(transfer, &bit_ofs, msg, true);
    return (bit_ofs+7)/8;
}
...
void _encode_uavcan_protocol_AccessCommandShell_req(uint8_t* buffer, uint32_t* bit_ofs, struct uavcan_protocol_AccessCommandShell_req_s* msg, bool tao) {
    ...
    *bit_ofs += 8;
    if (!tao) {
        canardEncodeScalar(buffer, *bit_ofs, 8, &msg->input_len);
        *bit_ofs += 8;
    }
    for (size_t i=0; i < msg->input_len; i++) {
            canardEncodeScalar(buffer, *bit_ofs, 8, &msg->input[i]);
            *bit_ofs += 8;
    }
}
...
void _decode_uavcan_protocol_AccessCommandShell_req(const CanardRxTransfer* transfer, uint32_t* bit_ofs, struct uavcan_protocol_AccessCommandShell_req_s* msg, bool tao) {
    ...
    *bit_ofs += 8;
    if (!tao) {
        canardDecodeScalar(transfer, *bit_ofs, 8, false, &msg->input_len);
        *bit_ofs += 8;
    }
    for (size_t i=0; i < msg->input_len; i++) {
            canardDecodeScalar(transfer, *bit_ofs, 8, false, &msg->input[i]);
            *bit_ofs += 8;
    }
}
